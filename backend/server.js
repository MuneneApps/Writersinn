...
// Static files (public folder recommended)
...
// Health check
app.get("/", (req, res) => {
  res.json({ message: "✅ WritersInn backend connected to Supabase" });
});
...
app.post("/take-task", async (req, res) => {
  try {
    ...
    if (assignments.some(a => ["pending", "completed"].includes(a.status))) {
    ...
    const { data: assignment, error: assignError } = await supabase
      .from("assignments")
      .insert([{ user_id: user.id, task_id: task.id, status: "pending", deadline: deadline.toISOString() }])
      .select()
      .single();
    ...
    await transporter.sendMail({
      ...
      html: `<p>Hello ${user.name},</p><p>Your task has been assigned. Check your WritersInn account to start.</p>`,
    });

    res.json({ message: "✅ Task assigned and email sent", assignment });
  } catch (err) {
...
app.post("/submit-task", upload.single("file"), async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<p>Hello ${user.name},</p><p>Your submission for <strong>${task.title}</strong> has been received. Balance updated.</p>`,
    });
...
app.post("/login", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<p>Hello ${user.name},</p><p>Click to login:</p><a href="${verifyUrl}">${verifyUrl}</a><p>Expires in 15 minutes.</p>`,
    });
